...
acmeInstallSSL() {
	if [[ "${dnsSSLStatus}" == "true" ]]; then
		...
		if [[ -n "${txtValue}" ]]; then
			...
			echoContent yellow " ---> 添加方法请参考此教程，https://github.com/mack-a/8in1/blob/master/documents/dns_txt.md"
...
nginxBlog() {
	...
	if [[ -d "/usr/share/nginx/html" && -f "/usr/share/nginx/html/check" ]]; then
		...
		if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
			...
			wget -q -P /usr/share/nginx https://raw.githubusercontent.com/mack-a/8in1/master/fodder/blog/unable/html${randomNum}.zip >/dev/null
...
customCDNIP() {
	...
	echoContent yellow "# 注意事项"
	echoContent yellow "\n教程地址:"
	echoContent skyBlue "https://github.com/mack-a/8in1/blob/master/documents/optimize_V2Ray.md"
...
updateNginxBlog() {
	...
	if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
		...
		if wget --help | grep -q show-progress; then
			wget -c -q --show-progress -P /usr/share/nginx "https://raw.githubusercontent.com/mack-a/8in1/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
		else
			wget -c -P /usr/share/nginx "https://raw.githubusercontent.com/mack-a/8in1/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
		fi
...
updateV2RayAgent() {
	...
	if wget --help | grep -q show-progress; then
		wget -c -q --show-progress -P /etc/8in1/ -N --no-check-certificate "https://raw.githubusercontent.com/mack-a/8in1/master/install.sh"
	else
		wget -c -q -P /etc/8in1/ -N --no-check-certificate "https://raw.githubusercontent.com/mack-a/8in1/master/install.sh"
	fi
	...
	echoContent skyBlue "wget -P /root -N --no-check-certificate https://raw.githubusercontent.com/mack-a/8in1/master/install.sh && chmod 700 /root/install.sh && /root/install.sh"
	echo
	exit 0
}
...
aliasInstall() {
	...
	if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/8in1" ]] && grep <"$HOME/install.sh" -q "作者:mack-a"; then
...
dokodemoDoorUnblockStreamingMedia() {
	...
	echoContent red "\n=============================================================="
	echoContent yellow "# 注意事项"
	echoContent yellow "任意门解锁详解，请查看此文章[https://github.com/mack-a/8in1/blob/master/documents/netflix/dokodemo-unblock_netflix.md]\n"
...
menu() {
	...
	echoContent green "Github:https://github.com/mack-a/8in1"
	echoContent green "描述:八合一共存脚本\c"
	showInstallStatus
	echoContent red "\n=============================================================="
	echoContent red "                        推广区                      "
	echoContent green "AFF捐赠：https://github.com/mack-a/8in1/blob/master/documents/donation_aff.md\n"
