...
initTLSNginxConfig() {
	...
	if [[ -n "${currentHost}" ]]; then
		...
		if [[ "${historyDomainStatus}" == "y" ]]; then
			...
			echo
			echoContent yellow "请输入要配置的域名 例: www.8in1.com --->"
			read -r -p "域名:" domain
		fi
	else
		echo
...
acmeInstallSSL() {
	if [[ "${dnsSSLStatus}" == "true" ]]; then
		...
		sudo "$HOME/.acme.sh/acme.sh" --issue -d "*.${dnsTLSDomain}" -d "${dnsTLSDomain}" --dns --yes-I-know-dns-manual-mode-enough-go-ahead-please -k ec-256 --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/8in1/tls/acme.log >/dev/null

		local txtValue=
		txtValue=$(tail -n 10 /etc/8in1/tls/acme.log | grep "TXT value" | awk -F "'" '{print $2}')
		if [[ -n "${txtValue}" ]]; then
			echoContent green " ---> 请手动添加DNS TXT记录"
			echoContent yellow " ---> 添加方法请参考此教程，https://github.com/izsiyy/8in1/blob/master/documents/dns_txt.md"
			...
			if [[ "${addDNSTXTRecordStatus}" == "y" ]]; then
				...
				if echo "${txtAnswer}" | grep -q "${txtValue}"; then
					...
					sudo "$HOME/.acme.sh/acme.sh" --renew -d "*.${dnsTLSDomain}" -d "${dnsTLSDomain}" --yes-I-know-dns-manual-mode-enough-go-ahead-please --ecc --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/8in1/tls/acme.log >/dev/null
				else
...
installTLS() {
	...
	if [[ -f "/etc/8in1/tls/${tlsDomain}.crt" && -f "/etc/8in1/tls/${tlsDomain}.key" && -n $(cat "/etc/8in1/tls/${tlsDomain}.crt") ]] || [[ -d "$HOME/.acme.sh/${tlsDomain}_ecc" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.key" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.cer" ]]; then
		...
		if [[ "${installDNSACMEStatus}" == "true" ]]; then
			echo
			if [[ -d "$HOME/.acme.sh/*.${dnsTLSDomain}_ecc" && -f "$HOME/.acme.sh/*.${dnsTLSDomain}_ecc/*.${dnsTLSDomain}.key" && -f "$HOME/.acme.sh/*.${dnsTLSDomain}_ecc/*.${dnsTLSDomain}.cer" ]]; then
				sudo "$HOME/.acme.sh/acme.sh" --installcert -d "*.${dnsTLSDomain}" --fullchainpath "/etc/8in1/tls/${tlsDomain}.crt" --keypath "/etc/8in1/tls/${tlsDomain}.key" --ecc >/dev/null
			fi
...
installV2Ray() {
	...
	if [[ "${coreInstallType}" != "2" && "${coreInstallType}" != "3" ]]; then
		...
		if [[ "${selectCoreType}" == "3" ]]; then
			...
			echoContent green " ---> v2ray-core版本:$(/etc/8in1/v2ray/v2ray --version | awk '{print $2}' | head -1)"
...
installHysteria() {
	...
	if [[ -z "${hysteriaConfigPath}" ]]; then
		...
		echoContent green " ---> Hysteria版本:$(/etc/8in1/hysteria/hysteria --version | awk '{print $3}')"
...
installXray() {
	...
	if [[ "${coreInstallType}" != "1" ]]; then
		...
		echoContent green " ---> Xray-core版本:$(/etc/8in1/xray/xray --version | awk '{print $2}' | head -1)"
...
updateV2Ray() {
	...
	if [[ -z "${coreInstallType}" ]]; then
		...
		echoContent green " ---> 当前v2ray-core版本:$(/etc/8in1/v2ray/v2ray --version | awk '{print $2}' | head -1)"
		...
		if [[ -n "$1" ]]; then
			...
			if [[ "${rollbackV2RayStatus}" == "y" ]]; then
				if [[ "${coreInstallType}" == "2" || "${coreInstallType}" == "3" ]]; then
					...
					echoContent green " ---> 当前Xray-core版本:$(/etc/8in1/xray/xray --version | awk '{print $2}' | head -1)"
				fi
...
handleV2Ray() {
	...
	if [[ "$1" == "start" ]]; then
		if [[ -n $(pgrep -f "v2ray/v2ray") ]]; then
			...
			echoContent red "请手动执行【/etc/8in1/v2ray/v2ray -confdir /etc/8in1/v2ray/conf】，查看错误日志"
			exit 0
		fi
	elif [[ "$1" == "stop" ]]; then
...
handleHysteria() {
	...
	if [[ "$1" == "start" ]]; then
		if [[ -n $(pgrep -f "hysteria/hysteria") ]]; then
			...
			echoContent red "请手动执行【/etc/8in1/hysteria/hysteria --log-level debug -c /etc/8in1/hysteria/conf/config.json server】，查看错误日志"
			exit 0
		fi
	elif [[ "$1" == "stop" ]]; then
...
handleXray() {
	...
	if [[ "$1" == "start" ]]; then
		if [[ -n $(pgrep -f "xray/xray") ]]; then
			...
			echoContent red "请手动执行【/etc/8in1/xray/xray -confdir /etc/8in1/xray/conf】，查看错误日志"
			exit 0
		fi
	elif [[ "$1" == "stop" ]]; then
...
unInstall() {
	...
	if [[ -d "/etc/8in1/tls" ]] && [[ -n $(find /etc/8in1/tls/ -name "*.key") ]] && [[ -n $(find /etc/8in1/tls/ -name "*.crt") ]]; then
		...
		if [[ -n $(find /tmp/8in1-tls -name '*.key') ]]; then
			echoContent yellow " ---> 备份证书成功，请注意留存。[/tmp/8in1-tls]"
		fi
	fi
...
updateV2RayAgent() {
	...
	local version
	version=$(grep '当前版本:v' "/etc/8in1/install.sh" | awk -F "[v]" '{print $2}' | tail -n +2 | head -n 1 | awk -F "[\"]" '{print $1}')
...
dokodemoDoorUnblockStreamingMedia() {
	...
	echoContent red "\n=============================================================="
	echoContent yellow "# 注意事项"
	echoContent yellow "任意门解锁详解，请查看此文章[https://github.com/izsiyy/8in1/blob/master/documents/netflix/dokodemo-unblock_netflix.md]\n"
...
menu() {
	...
	echoContent red "\n=============================================================="
	echoContent red "                        推广区                      "
	echoContent green "AFF捐赠：https://github.com/izsiyy/8in1/blob/master/documents/donation_aff.md\n"
