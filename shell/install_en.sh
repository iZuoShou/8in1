...
readInstallType() {
	...
	if [[ -d "/etc/8in1" ]]; then
		...
		if [[ -d "/etc/8in1/v2ray" && -f "/etc/8in1/v2ray/v2ray" && -f "/etc/8in1/v2ray/v2ctl" ]]; then
			if [[ -d "/etc/8in1/v2ray/conf" && -f "/etc/8in1/v2ray/conf/02_VLESS_TCP_inbounds.json" ]]; then
				configPath=/etc/8in1/v2ray/conf/
				if grep </etc/8in1/v2ray/conf/02_VLESS_TCP_inbounds.json -q '"security": "tls"'; then
					# v2ray-core without XTLS
					coreInstallType=2
					ctlPath=/etc/8in1/v2ray/v2ctl
				elif grep </etc/8in1/v2ray/conf/02_VLESS_TCP_inbounds.json -q '"security": "xtls"'; then
		...
		if [[ -d "/etc/8in1/xray" && -f "/etc/8in1/xray/xray" ]]; then
			# Check xray-core here
			if [[ -d "/etc/8in1/xray/conf" ]] && [[ -f "/etc/8in1/xray/conf/02_VLESS_TCP_inbounds.json" || -f "/etc/8in1/xray/conf/02_trojan_TCP_inbounds.json" ]]; then
				# xray-core
				configPath=/etc/8in1/xray/conf/
				ctlPath=/etc/8in1/xray/xray
				coreInstallType=1
			fi
		fi

		if [[ -d "/etc/8in1/hysteria" && -f "/etc/8in1/hysteria/hysteria" ]]; then
			# Check hysteria here
			if [[ -d "/etc/8in1/hysteria/conf" ]] && [[ -f "/etc/8in1/hysteria/conf/config.json" ]] && [[ -f "/etc/8in1/hysteria/conf/client_network.json" ]]; then
				hysteriaConfigPath=/etc/8in1/hysteria/conf/
			fi
		fi
...
allowPort() {
	...
	if systemctl status netfilter-persistent 2>/dev/null | grep -q "active (exited)"; then
		...
		if ! iptables -L | grep -q "$1(izsiyy)"; then
			updateFirewalldStatus=true
			iptables -I INPUT -p tcp --dport "$1" -m comment --comment "allow $1(izsiyy)" -j ACCEPT
		fi
...
cleanUp() {
	if [[ "$1" == "v2rayClean" ]]; then
		rm -rf "$(find /etc/8in1/v2ray/* | grep -E '(config_full.json|conf)')"
		handleV2Ray stop >/dev/null
		rm -f /etc/systemd/system/v2ray.service
	elif [[ "$1" == "xrayClean" ]]; then
		rm -rf "$(find /etc/8in1/xray/* | grep -E '(config_full.json|conf)')"
		handleXray stop >/dev/null
		rm -f /etc/systemd/system/xray.service

	elif [[ "$1" == "v2rayDel" ]]; then
		rm -rf /etc/8in1/v2ray/*
...
mkdirTools() {
	...
	mkdir -p /etc/8in1/subscribe
	mkdir -p /etc/8in1/subscribe_tmp
	mkdir -p /etc/8in1/v2ray/conf
	mkdir -p /etc/8in1/v2ray/tmp
	mkdir -p /etc/8in1/xray/conf
	mkdir -p /etc/8in1/xray/tmp
	mkdir -p /etc/8in1/trojan
	mkdir -p /etc/8in1/hysteria/conf
	mkdir -p /etc/systemd/system/
	mkdir -p /tmp/8in1-tls/
}
...
installTools() {
	...
	${upgrade} >/etc/8in1/install.log 2>&1
	if grep <"/etc/8in1/install.log" -q "changed"; then
	...
	if [[ ! -d "$HOME/.acme.sh" ]] || [[ -d "$HOME/.acme.sh" && -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
		...
		curl -s https://get.acme.sh | sh >/etc/8in1/tls/acme.log 2>&1

		if [[ ! -d "$HOME/.acme.sh" ]] || [[ -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
			echoContent red "acme installation failed--->"
			tail -n 100 /etc/8in1/tls/acme.log
...
initTLSNginxConfig() {
	...
	if [[ -n "${currentHost}" ]]; then
		...
		if [[ "${historyDomainStatus}" == "y" ]]; then
			...
			echo
			echoContent yellow "Please enter the domain name to be configured Example: www.8in1.com --->"
...
switchSSLType() {
	if [[ -z "${sslType}" ]]; then
		...
		*)
		...
		touch /etc/8in1/tls
		echo "${sslType}" >/etc/8in1/tls/ssl_type
...
acmeInstallSSL() {
	if [[ "${dnsSSLStatus}" == "true" ]]; then
		...
		sudo "$HOME/.acme.sh/acme.sh" --issue -d "*.${dnsTLSDomain}" --dns --yes-I-know-dns-manual-mode-enough-go-ahead-please --standalone -k ec-256 --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/8in1/tls/acme.log >/dev/null

		local txtValue=
		txtValue=$(tail -n 10 /etc/8in1/tls/acme.log | grep "TXT value" | awk -F "'" '{print $2}')
		if [[ -n "${txtValue}" ]]; then
			echoContent green " ---> Please manually add DNS TXT record"
			echoContent yellow " ---> Please refer to this tutorial for adding methods, https://github.com/izsiyy/8in1/blob/master/documents/dns_txt.md"
			...
			if [[ "${addDNSTXTRecordStatus}" == "y" ]]; then
				...
				if [[ "${txtAnswer}" == "${txtValue}" ]]; then
					...
					sudo "$HOME/.acme.sh/acme.sh" --renew -d "*.${dnsTLSDomain}" --yes-I-know-dns-manual-mode-enough-go-ahead-please --ecc --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/8in1/tls/acme.log >/dev/null
				else
		...
		echoContent green "---> Generating certificate"
		sudo "$HOME/.acme.sh/acme.sh" --issue -d "${tlsDomain}" --standalone -k ec-256 --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/8in1/tls/acme.log >/dev/null
	fi
}
# custom port
customPortFunction() {
...
installTLS() {
	...
	# install tls
	if [[ -f "/etc/8in1/tls/${tlsDomain}.crt" && -f "/etc/8in1/tls/${tlsDomain}.key" && -n $(cat "/etc/8in1/tls/${tlsDomain}.crt") ]] || [[ -d "$HOME/.acme.sh/${tlsDomain}_ecc" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.key" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.cer" ]]; then
		echoContent green " ---> Certificate detected"
		# checkTLStatus
		renewalTLS

		if [[ -z $(find /etc/8in1/tls/ -name "${tlsDomain}.crt") ]] || [[ -z $(find /etc/8in1/tls/ -name "${tlsDomain}.key") ]] || [[ -z $(cat "/etc/8in1/tls/${tlsDomain}.crt") ]]; then
			sudo "$HOME/.acme.sh/acme.sh" --installcert -d "${tlsDomain}" --fullchainpath "/etc/8in1/tls/${tlsDomain}.crt" --keypath "/etc/8in1/tls/${tlsDomain}.key" --ecc >/dev/null
		else
		...
		if [[ "${installDNSACMEStatus}" == "true" ]]; then
			echo
			if [[ -d "$HOME/.acme.sh/*.${dnsTLSDomain}_ecc" && -f "$HOME/.acme.sh/*.${dnsTLSDomain}_ecc/*.${dnsTLSDomain}.key" && -f "$HOME/.acme.sh/*.${dnsTLSDomain}_ecc/*.${dnsTLSDomain}.cer" ]]; then
				sudo "$HOME/.acme.sh/acme.sh" --installcert -d "${dnsTLSDomain}" -d "*.${dnsTLSDomain}" --fullchainpath "/etc/8in1/tls/${tlsDomain}.crt" --keypath "/etc/8in1/tls/${tlsDomain}.key" --ecc >/dev/null
			fi
		...
		if [[ ! -f "/etc/8in1/tls/${tlsDomain}.crt" || ! -f "/etc/8in1/tls/${tlsDomain}.key" ]] || [[ -z $(cat "/etc/8in1/tls/${tlsDomain}.key") || -z $(cat "/etc/8in1/tls/${tlsDomain}.crt") ]]; then
			tail -n 10 /etc/8in1/tls/acme.log
			...
			if tail -n 10 /etc/8in1/tls/acme.log | grep -q "Could not validate email address as valid"; then
...
nginxBlog() {
	...
	if [[ -d "/usr/share/nginx/html" && -f "/usr/share/nginx/html/check" ]]; then
		...
		if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
			...
			wget -q -P /usr/share/nginx https://raw.githubusercontent.com/izsiyy/8in1/master/fodder/blog/unable/html${randomNum}.zip >/dev/null
...
updateSELinuxHTTPPortT() {
	...
	$(find /usr/bin /usr/sbin | grep -w journalctl) -xe >/etc/8in1/nginx_error.log 2>&1

	if find /usr/bin /usr/sbin | grep -q -w semanage && find /usr/bin /usr/sbin | grep -q -w getenforce && grep -E "31300|31302" </etc/8in1/nginx_error.log | grep -q "Permission denied"; then
...
handleNginx() {
	...
	if [[ -z $(pgrep -f "nginx") ]] && [[ "$1" == "start" ]]; then
		systemctl start nginx 2>/etc/8in1/nginx_error.log
		...
		if [[ -z $(pgrep -f nginx) ]]; then
			...
			if grep -q "journalctl -xe" </etc/8in1/nginx_error.log; then
				updateSELinuxHTTPPortT
			fi
...
installCronTLS() {
	...
	crontab -l >/etc/8in1/backup_crontab.cron
	local historyCrontab
	historyCrontab=$(sed '/8in1/d;/acme.sh/d' /etc/8in1/backup_crontab.cron)
	echo "${historyCrontab}" >/etc/8in1/backup_crontab.cron
	echo "30 1 * * * /bin/bash /etc/8in1/install.sh RenewTLS >> /etc/8in1/crontab_tls.log 2>&1" >>/etc/8in1/backup_crontab.cron
	crontab /etc/8in1/backup_crontab.cron
...
renewalTLS() {
	...
	if [[ -f "/etc/8in1/tls/ssl_type" ]]; then
		if grep -q "buypass" <"/etc/8in1/tls/ssl_type"; then
			sslRenewalDays=180
		fi
	fi
	if [[ -d "$HOME/.acme.sh/${domain}_ecc" && -f "$HOME/.acme.sh/${domain}_ecc/${domain}.key" && -f "$HOME/.acme.sh/${domain}_ecc/${domain}.cer" ]] || [[ "${installDNSACMEStatus}" == "true" ]]; then
		...
		if [[ ${remainingDays} -le 1 ]]; then
			...
			sudo "$HOME/.acme.sh/acme.sh" --installcert -d "${domain}" --fullchainpath /etc/8in1/tls/"${domain}.crt" --keypath /etc/8in1/tls/"${domain}.key" --ecc
			reloadCore
			handleNginx start
		else
...
installV2Ray() {
	...
	if [[ "${coreInstallType}" != "2" && "${coreInstallType}" != "3" ]]; then
		...
		if wget --help | grep -q show-progress; then
			wget -c -q --show-progress -P /etc/8in1/v2ray/ "https://github.com/v2fly/v2ray-core/releases/download/${version}/${v2rayCoreCPUVendor}.zip"
		else
			wget -c -P /etc/8in1/v2ray/ "https://github.com/v2fly/v2ray-core/releases/download/${version}/${v2rayCoreCPUVendor}.zip" >/dev/null 2>&1
		fi

		unzip -o "/etc/8in1/v2ray/${v2rayCoreCPUVendor}.zip" -d /etc/8in1/v2ray >/dev/null
		rm -rf "/etc/8in1/v2ray/${v2rayCoreCPUVendor}.zip"
	else
		if [[ "${selectCoreType}" == "3" ]]; then
			echoContent green " ---> lock v2ray-core version to v4.32.1"
			rm -f /etc/8in1/v2ray/v2ray
			rm -f /etc/8in1/v2ray/v2ctl
			installV2Ray "$1"
		else
			echoContent green " ---> v2ray-core version:$(/etc/8in1/v2ray/v2ray --version | awk '{print $2}' | head -1)"
...
installHysteria() {
	...
	if [[ -z "${hysteriaConfigPath}" ]]; then
		...
		if wget --help | grep -q show-progress; then
			wget -c -q --show-progress -P /etc/8in1/hysteria/ "https://github.com/HyNetwork/hysteria/releases/download/${version}/${hysteriaCoreCPUVendor}"
		else
			wget -c -P /etc/8in1/hysteria/ "https://github.com/HyNetwork/hysteria/releases/download/${version}/${hysteriaCoreCPUVendor}" >/dev/null 2>&1
		fi
		mv "/etc/8in1/hysteria/${hysteriaCoreCPUVendor}" /etc/8in1/hysteria/hysteria
		chmod 655 /etc/8in1/hysteria/hysteria
	else
		echoContent green " ---> Hysteria version:$(/etc/8in1/hysteria/hysteria --version | awk '{print $3}')"
		read -r -p "Update, upgrade? [y/n]:" reInstallHysteriaStatus
		if [[ "${reInstallHysteriaStatus}" == "y" ]]; then
			rm -f /etc/8in1/hysteria/hysteria
			installHysteria "$1"
		fi
	fi
...
installXray() {
	...
	if [[ "${coreInstallType}" != "1" ]]; then
		...
		if wget --help | grep -q show-progress; then
			wget -c -q --show-progress -P /etc/8in1/xray/ "https://github.com/XTLS/Xray-core/releases/download/${version}/${xrayCoreCPUVendor}.zip"
		else
			wget -c -P /etc/8in1/xray/ "https://github.com/XTLS/Xray-core/releases/download/${version}/${xrayCoreCPUVendor}.zip" >/dev/null 2>&1
		fi

		unzip -o "/etc/8in1/xray/${xrayCoreCPUVendor}.zip" -d /etc/8in1/xray >/dev/null
		rm -rf "/etc/8in1/xray/${xrayCoreCPUVendor}.zip"
		chmod 655 /etc/8in1/xray/xray
	else
		echoContent green " ---> Xray-core version:$(/etc/8in1/xray/xray --version | awk '{print $2}' | head -1)"
		read -r -p "Update, upgrade? [y/n]:" reInstallXrayStatus
		if [[ "${reInstallXrayStatus}" == "y" ]]; then
			rm -f /etc/8in1/xray/xray
			installXray "$1"
		fi
	fi
}
...
updateV2Ray() {
	...
	if [[ -z "${coreInstallType}" ]]; then
		...
		echoContent green " ---> Current v2ray-core version: $(/etc/8in1/v2ray/v2ray --version | awk '{print $2}' | head -1)"
		...
		if [[ -n "$1" ]]; then
			...
			if [[ "${rollbackV2RayStatus}" == "y" ]]; then
				if [[ "${coreInstallType}" == "2" || "${coreInstallType}" == "3" ]]; then
					...
					echoContent green " ---> Current Xray-core version: $(/etc/8in1/xray/xray --version | awk '{print $2}' | head -1)"
				fi
		...
		elif [[ "${version}" == "v$(/etc/8in1/v2ray/v2ray --version | awk '{print $2}' | head -1)" ]]; then
...
updateXray() {
	...
	if [[ -z "${coreInstallType}" ]]; then
		...
		elif [[ "${version}" == "v$(/etc/8in1/xray/xray --version | awk '{print $2}' | head -1)" ]]; then
...
installV2RayService() {
	...
	if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
		...
		execStart='/etc/8in1/v2ray/v2ray -confdir /etc/8in1/v2ray/conf'
...
installHysteriaService() {
	...
	if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
		...
		execStart='/etc/8in1/hysteria/hysteria --log-level info -c /etc/8in1/hysteria/conf/config.json server'
...
installXrayService() {
	...
	if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
		...
		execStart='/etc/8in1/xray/xray run -confdir /etc/8in1/xray/conf'
...
handleV2Ray() {
	...
	if [[ "$1" == "start" ]]; then
		if [[ -n $(pgrep -f "v2ray/v2ray") ]]; then
			...
			echoContent red "Please manually execute [/etc/8in1/v2ray/v2ray -confdir /etc/8in1/v2ray/conf] to view the error log"
			exit 0
		fi
	elif [[ "$1" == "stop" ]]; then
...
handleHysteria() {
	...
	if [[ "$1" == "start" ]]; then
		if [[ -n $(pgrep -f "hysteria/hysteria") ]]; then
			...
			echoContent red "Please manually execute [/etc/8in1/hysteria/hysteria --log-level debug -c /etc/8in1/hysteria/conf/config.json server] to view the error log"
			exit 0
		fi
	elif [[ "$1" == "stop" ]]; then
...
handleXray() {
	...
	if [[ "$1" == "start" ]]; then
		if [[ -n $(pgrep -f "xray/xray") ]]; then
			...
			echoContent red "Please manually execute [/etc/8in1/xray/xray -confdir /etc/8in1/xray/conf] to view the error log"
			exit 0
		fi
	elif [[ "$1" == "stop" ]]; then
...
initHysteriaNetwork() {
	...
	cat <<EOF >/etc/8in1/hysteria/conf/client_network.json
{
...
initHysteriaConfig() {
	...
	cat <<EOF >/etc/8in1/hysteria/conf/config.json
{
	"listen": ":${hysteriaPort}",
	"protocol": "${hysteriaProtocol}",
	"disable_udp": false,
	"cert": "/etc/8in1/tls/${currentHost}.crt",
	"key": "/etc/8in1/tls/${currentHost}.key",
	"auth": {
		"mode": "passwords",
		"config": []
	},
...
EOF
	...
	addClientsHysteria "/etc/8in1/hysteria/conf/config.json" true
}
...
initV2RayConfig() {
	...
	if [[ -n "${currentUUID}" && -z "${uuid}" ]]; then
		...
		if [[ "${historyUUIDStatus}" == "y" ]]; then
			...
			uuid=$(/etc/8in1/v2ray/v2ctl uuid)
		fi
	elif [[ -z "${uuid}" ]]; then
		uuid=$(/etc/8in1/v2ray/v2ctl uuid)
	fi
	...
	movePreviousConfig
	# log
	cat <<EOF >/etc/8in1/v2ray/conf/00_log.json
{
  "log": {
    "error": "/etc/8in1/v2ray/error.log",
    "loglevel": "warning"
  }
}
EOF
	# outbounds
	if [[ -n "${pingIPv6}" ]]; then
		cat <<EOF >/etc/8in1/v2ray/conf/10_ipv6_outbounds.json
{
    "outbounds": [
        {
          "protocol": "freedom",
...
EOF
		...
		cat <<EOF >/etc/8in1/v2ray/conf/10_ipv4_outbounds.json
{
    "outbounds":[
        {
            "protocol":"freedom",
...
EOF
	...
	# dns
	cat <<EOF >/etc/8in1/v2ray/conf/11_dns.json
{
    "dns": {
        "servers": [
          "localhost"
...
EOF
	...
	if echo "${selectCustomInstallType}" | grep -q 4 || [[ "$1" == "all" ]]; then
		...
		cat <<EOF >/etc/8in1/v2ray/conf/04_trojan_TCP_inbounds.json
{
"inbounds":[
	{
	  "port": 31296,
...
EOF
		addClients "/etc/8in1/v2ray/conf/04_trojan_TCP_inbounds.json" "${addClientsStatus}"
	fi
	...
	if echo "${selectCustomInstallType}" | grep -q 1 || [[ "$1" == "all" ]]; then
		...
		cat <<EOF >/etc/8in1/v2ray/conf/03_VLESS_WS_inbounds.json
{
"inbounds":[
    {
	  "port": 31297,
...
EOF
		addClients "/etc/8in1/v2ray/conf/03_VLESS_WS_inbounds.json" "${addClientsStatus}"
	fi
	...
	if echo "${selectCustomInstallType}" | grep -q 2 || [[ "$1" == "all" ]]; then
		...
		cat <<EOF >/etc/8in1/v2ray/conf/04_trojan_gRPC_inbounds.json
{
    "inbounds": [
        {
            "port": 31304,
...
EOF
		addClients "/etc/8in1/v2ray/conf/04_trojan_gRPC_inbounds.json" "${addClientsStatus}"
	fi
	...
	if echo "${selectCustomInstallType}" | grep -q 3 || [[ "$1" == "all" ]]; then
		...
		cat <<EOF >/etc/8in1/v2ray/conf/05_VMess_WS_inbounds.json
{
"inbounds":[
{
  "listen": "127.0.0.1",
...
EOF
		addClients "/etc/8in1/v2ray/conf/05_VMess_WS_inbounds.json" "${addClientsStatus}"
	fi

	if echo "${selectCustomInstallType}" | grep -q 5 || [[ "$1" == "all" ]]; then
		getClients "${configPath}../tmp/06_VLESS_gRPC_inbounds.json" "${addClientsStatus}"
		cat <<EOF >/etc/8in1/v2ray/conf/06_VLESS_gRPC_inbounds.json
{
    "inbounds":[
    {
        "port": 31301,
...
EOF
		addClients "/etc/8in1/v2ray/conf/06_VLESS_gRPC_inbounds.json" "${addClientsStatus}"
	fi
	...
	cat <<EOF >/etc/8in1/v2ray/conf/02_VLESS_TCP_inbounds.json
{
"inbounds":[
{
  "port": ${defaultPort},
  ...
  "streamSettings": {
    ...
    "tlsSettings": {
      ...
      "certificates": [
        {
          "certificateFile": "/etc/8in1/tls/${domain}.crt",
          "keyFile": "/etc/8in1/tls/${domain}.key",
          "ocspStapling": 3600,
          "usage":"encipherment"
        }
      ]
...
EOF
	addClients "/etc/8in1/v2ray/conf/02_VLESS_TCP_inbounds.json" "${addClientsStatus}"
...
initXrayConfig() {
	...
	# log
	cat <<EOF >/etc/8in1/xray/conf/00_log.json
{
  "log": {
    "error": "/etc/8in1/xray/error.log",
    "loglevel": "warning"
  }
}
EOF
	...
	# outbounds
	if [[ -n "${pingIPv6}" ]]; then
		cat <<EOF >/etc/8in1/xray/conf/10_ipv6_outbounds.json
{
    "outbounds": [
        {
          "protocol": "freedom",
...
EOF
		...
		cat <<EOF >/etc/8in1/xray/conf/10_ipv4_outbounds.json
{
    "outbounds":[
        {
            "protocol":"freedom",
...
EOF
	...
	# dns
	cat <<EOF >/etc/8in1/xray/conf/11_dns.json
{
    "dns": {
        "servers": [
          "localhost"
...
EOF
	...
	if echo "${selectCustomInstallType}" | grep -q 4 || [[ "$1" == "all" ]]; then
		...
		cat <<EOF >/etc/8in1/xray/conf/04_trojan_TCP_inbounds.json
{
"inbounds":[
	{
	  "port": 31296,
...
EOF
		addClients "/etc/8in1/xray/conf/04_trojan_TCP_inbounds.json" "${addClientsStatus}"
	fi
	...
	if echo "${selectCustomInstallType}" | grep -q 1 || [[ "$1" == "all" ]]; then
		fallbacksList=${fallbacksList}',{"path":"/'${customPath}'ws","dest":31297,"xver":1}'
		getClients "${configPath}../tmp/03_VLESS_WS_inbounds.json" "${addClientsStatus}"
		cat <<EOF >/etc/8in1/xray/conf/03_VLESS_WS_inbounds.json
{
"inbounds":[
    {
	  "port": 31297,
...
EOF
		addClients "/etc/8in1/xray/conf/03_VLESS_WS_inbounds.json" "${addClientsStatus}"
	fi
	...
	if echo "${selectCustomInstallType}" | grep -q 2 || [[ "$1" == "all" ]]; then
		...
		getClients "${configPath}../tmp/04_trojan_gRPC_inbounds.json" "${addClientsStatus}"
		cat <<EOF >/etc/8in1/xray/conf/04_trojan_gRPC_inbounds.json
{
    "inbounds": [
        {
            "port": 31304,
...
EOF
		addClients "/etc/8in1/xray/conf/04_trojan_gRPC_inbounds.json" "${addClientsStatus}"
	fi
	...
	if echo "${selectCustomInstallType}" | grep -q 3 || [[ "$1" == "all" ]]; then
		fallbacksList=${fallbacksList}',{"path":"/'${customPath}'vws","dest":31299,"xver":1}'
		getClients "${configPath}../tmp/05_VMess_WS_inbounds.json" "${addClientsStatus}"
		cat <<EOF >/etc/8in1/xray/conf/05_VMess_WS_inbounds.json
{
"inbounds":[
{
  "listen": "127.0.0.1",
...
EOF
		addClients "/etc/8in1/xray/conf/05_VMess_WS_inbounds.json" "${addClientsStatus}"
	fi

	if echo "${selectCustomInstallType}" | grep -q 5 || [[ "$1" == "all" ]]; then
		getClients "${configPath}../tmp/06_VLESS_gRPC_inbounds.json" "${addClientsStatus}"
		cat <<EOF >/etc/8in1/xray/conf/06_VLESS_gRPC_inbounds.json
{
    "inbounds":[
    {
        "port": 31301,
...
EOF
		addClients "/etc/8in1/xray/conf/06_VLESS_gRPC_inbounds.json" "${addClientsStatus}"
	fi
	...
	cat <<EOF >/etc/8in1/xray/conf/02_VLESS_TCP_inbounds.json
{
"inbounds":[
{
  "port": ${defaultPort},
...
EOF
	addClients "/etc/8in1/xray/conf/02_VLESS_TCP_inbounds.json" "${addClientsStatus}"
}
...
initTrojanGoConfig() {
	...
	cat <<EOF >/etc/8in1/trojan/config_full.json
{
    "run_type": "server",
    ...
    "log_level":3,
    "log_file":"/etc/8in1/trojan/trojan.log",
    "password": [
        "${uuid}"
    ],
    "dns":[
...
customCDNIP() {
	...
	echoContent yellow "# Notes"
	echoContent yellow "\nTutorial address:"
	echoContent skyBlue "https://github.com/izsiyy/8in1/blob/master/documents/optimize_V2Ray.md"
...
defaultBase64Code() {
	...
	if [[ "${type}" == "vlesstcp" ]]; then
		...
		if [[ "${coreInstallType}" == "1" ]] && echo "${currentInstallProtocolType}" | grep -q 0; then
			...
			cat <<EOF >>"/etc/8in1/subscribe_tmp/${subAccount}"
...
backupNginxConfig() {
	if [[ "$1" == "backup" ]]; then
		cp /etc/nginx/conf.d/alone.conf /etc/8in1/alone_backup.conf
		echoContent green " ---> nginx configuration file backup successfully"
	fi

	if [[ "$1" == "restoreBackup" ]] && [[ -f "/etc/8in1/alone_backup.conf" ]]; then
		cp /etc/8in1/alone_backup.conf /etc/nginx/conf.d/alone.conf
		echoContent green " ---> nginx configuration file restore and backup successfully"
		rm /etc/8in1/alone_backup.conf
	fi
...
updateNginxBlog() {
	...
	if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
		...
		if wget --help | grep -q show-progress; then
			wget -c -q --show-progress -P /usr/share/nginx "https://raw.githubusercontent.com/izsiyy/8in1/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
		else
			wget -c -P /usr/share/nginx "https://raw.githubusercontent.com/izsiyy/8in1/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
		fi
...
unInstall() {
	...
	rm -rf /tmp/8in1-tls/*
	if [[ -d "/etc/8in1/tls" ]] && [[ -n $(find /etc/8in1/tls/ -name "*.key") ]] && [[ -n $(find /etc/8in1/tls/ -name "*.crt") ]]; then
		mv /etc/8in1/tls /tmp/8in1-tls
		if [[ -n $(find /tmp/8in1-tls -name '*.key') ]]; then
			echoContent yellow " ---> The backup certificate is successful, please keep it.[/tmp/8in1-tls]"
		fi
	fi
	...
	echoContent green "---> Uninstall 8in1 script completed"
}
...
customUUID() {
	...
	if [[ -z "${currentCustomUUID}" ]]; then
		...
		if [[ -f "/tmp/8in1" && -n $(cat /tmp/8in1) ]]; then
...
# Updated script
updateV2RayAgent() {
	echoContent skyBlue "\nProgress $1/${totalProgress} : Update 8in1 script"
	rm -rf /etc/8in1/install.sh
	if wget --help | grep -q show-progress; then
		wget -c -q --show-progress -P /etc/8in1/ -N --no-check-certificate "https://raw.githubusercontent.com/izsiyy/8in1/master/install.sh"
	else
		wget -c -q -P /etc/8in1/ -N --no-check-certificate "https://raw.githubusercontent.com/izsiyy/8in1/master/install.sh"
	fi

	sudo chmod 700 /etc/8in1/install.sh
	local version
	version=$(grep 'Current version:v' "/etc/8in1/install.sh" | awk -F "[v]" '{print $2}' | tail -n +2 | head -n 1 | awk -F "[\"]" '{print $1}')
	...
	echoContent skyBlue "wget -P /root -N --no-check-certificate https://raw.githubusercontent.com/izsiyy/8in1/master/install.sh && chmod 700 /root/install.sh && /root/install.sh"
	echo
	exit 0
}
...
EOF
	...
	3)
		...
		;;
	4)
		tail -n 100 /etc/8in1/crontab_tls.log
		;;
	5)
		tail -n 100 /etc/8in1/tls/acme.log
		;;
	...
	if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/8in1" ]] && grep <"$HOME/install.sh" -q "作者:izsiyy"; then
		...
		if [[ -d "/usr/bin/" ]]; then
			if [[ ! -f "/usr/bin/vasma" ]]; then
				ln -s /etc/8in1/install.sh /usr/bin/vasma
			...
			if [[ ! -f "/usr/sbin/vasma" ]]; then
				ln -s /etc/8in1/install.sh /usr/sbin/vasma
...
dokodemoDoorUnblockStreamingMedia() {
	...
	echoContent red "\n=============================================================="
	echoContent yellow "# Notes"
	echoContent yellow "Any door unlock details, please check this article [https://github.com/izsiyy/8in1/blob/master/documents/netflix/dokodemo-unblock_netflix.md]\n"
...
subscribe() {
	if [[ -n "${configPath}" ]]; then
		...
		rm -rf /etc/8in1/subscribe/*
		rm -rf /etc/8in1/subscribe_tmp/*
		showAccounts >/dev/null
		mv /etc/8in1/subscribe_tmp/* /etc/8in1/subscribe/

		if [[ -n $(ls /etc/8in1/subscribe/) ]]; then
			find /etc/8in1/subscribe/* | while read -r email; do
				email=$(echo "${email}" | awk -F "[b][e][/]" '{print $2}')

				local base64Result
				base64Result=$(base64 -w 0 "/etc/8in1/subscribe/${email}")
				echo "${base64Result}" >"/etc/8in1/subscribe/${email}"
				echoContent skyBlue "--------------------------------------------------------------"
...
menu() {
	cd "$HOME" || exit
	echoContent red "\n=============================================================="
	echoContent green "author:izsiyy"
	...
	echoContent green "AFF donation: https://github.com/izsiyy/8in1/blob/master/documents/donation_aff.md\n"
